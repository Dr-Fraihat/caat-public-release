...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const idList = tabs.map(t => t.getAttribute("data-tab")).filter(Boolean);

  function showById(id) {
    // hide all in this group
    idList.forEach(cid => document.getElementById(cid)?.classList.remove("active"));
    tabs.forEach(t => t.classList.remove("active"));
    // activate chosen
    const targetTab = tabs.find(t => t.getAttribute("data-tab") === id);
  ...
  // click handlers
  tabs.forEach(tab => {
    tab.addEventListener("click", () => showById(tab.getAttribute("data-tab")));
  });

  // default: first tab that actually has a container
  const firstWithContainer = idList.find(cid => document.getElementById(cid));
  if (firstWithContainer) showById(firstWithContainer);
});
...
});  // <— close the .forEach(tabGroup) here
...
const deliverySelect = document.getElementById("deliveryMode");
if (deliverySelect) {
  deliverySelect.addEventListener("change", () => {
    const v = deliverySelect.value;
    toggleDisplay("csectionReasonWrapper", v === "c-section");
    toggleDisplay("otherDeliveryWrapper", v === "other");
  });
  // set state on load
  const v = deliverySelect.value;
  toggleDisplay("csectionReasonWrapper", v === "c-section");
...
const API_BASE =
  (location.hostname === '127.0.0.1' || location.hostname === 'localhost')
    ...
    // === CAAT-PSYCH: simple wrapper to reuse the AI pipeline ===
async function generatePsychReport(payload){
  window.generatedReportData = { ...payload, meta: { reportType: "PSYCH" } };
...
if (
  (reportType === 'OT'    && result.templateUsed !== 'ot') ||
  (reportType === 'PSYCH' && result.templateUsed !== 'caat-psych') ||
  (reportType === 'ADIR'  && result.templateUsed !== 'adir')
) {
...
const __signupForm = document.getElementById("signupForm");
if (__signupForm) {
  __signupForm.addEventListener("submit", async function(e) {
    e.preventDefault();
    ...
    try {
      ...
      if (userDoc.exists() && userDoc.data().isActive === true) return;
      const res = await fetch("https://caat-backend.onrender.com/create-checkout-session", { method: "POST", headers: { "Content-Type": "application/json" }});
      const data = await res.json();
      if (data.url) window.location.href = data.url; else status.textContent = "❌ Failed to redirect to payment page.";
    } catch (err) {
      console.error(err);
...
const __resetForm = document.getElementById("resetForm");
if (__resetForm) {
  __resetForm.addEventListener("submit", async function(e) {
    e.preventDefault();
...
/* ================= UNIFIED PSYCH ASSESSMENT ================= */

const U = {
  el: (s,r=document)=>r.querySelector(s),
  els:(s,r=document)=>Array.from(r.querySelectorAll(s)),
  val:(e)=>e && typeof e.value==='string' ? e.value.trim() : e?.value ?? '',
};

// ---- 2.1 Item-bank placeholders ----
// Replace PROMPTS with your licensed wording if you have the rights;
// the structure stays the same. Shared items are asked once.
const ITEM_BANKS = {
  shared: [
    { id:'eye_contact',    type:'likert_0_3', domain:'social',    prompt:'Spontaneous eye contact' },
    { id:'joint_attention',type:'likert_0_3', domain:'social',    prompt:'Shares enjoyment / joint attention' },
    { id:'echolalia',      type:'likert_0_3', domain:'comm',      prompt:'Echolalia or repetitive speech' },
    { id:'rrb_general',    type:'likert_0_3', domain:'rrb',       prompt:'Restricted / repetitive behaviors' },
    { id:'play_imagin',    type:'likert_0_3', domain:'play',      prompt:'Imaginative / pretend play' },
  ],
  // Instrument-specific sections; keep only items not already in "shared".
  cars2: [
    { id:'cars_relating', type:'cars_1_4', prompt:'Relating to people' },
    { id:'cars_verbal',   type:'cars_1_4', prompt:'Verbal communication' },
    // …add remaining CARS items you’re licensed to display…
  ],
  ados2: {
    modules:['Toddler','Module 1','Module 2','Module 3','Module 4'],
    items: [
      { id:'ados_comm',   type:'num_0_3', prompt:'Communication' },
      { id:'ados_social', type:'num_0_3', prompt:'Reciprocal social interaction' },
      { id:'ados_play',   type:'num_0_3', prompt:'Play / Imagination' },
      { id:'ados_rrb',    type:'num_0_3', prompt:'Stereotyped & restricted interests' },
    ]
  },
  wisc: [
    { id:'wisc_vci',  type:'std_40_160', prompt:'VCI' },
    { id:'wisc_vsi',  type:'std_40_160', prompt:'VSI' },
    { id:'wisc_fri',  type:'std_40_160', prompt:'FRI' },
    { id:'wisc_wmi',  type:'std_40_160', prompt:'WMI' },
    { id:'wisc_psi',  type:'std_40_160', prompt:'PSI' },
    { id:'wisc_fsiq', type:'std_40_160', prompt:'FSIQ' },
  ],
  sb5: [
    { id:'sb5_fr_v', type:'scaled_1_19', prompt:'Fluid Reasoning (Verbal)' },
    { id:'sb5_fr_nv',type:'scaled_1_19', prompt:'Fluid Reasoning (Nonverbal)' },
    { id:'sb5_kn_v', type:'scaled_1_19', prompt:'Knowledge (Verbal)' },
    { id:'sb5_kn_nv',type:'scaled_1_19', prompt:'Knowledge (Nonverbal)' },
    // …add remaining SB-5 subtests…
  ],
  gars3: [
    { id:'gars_si',  type:'std_40_160', prompt:'Social Interaction (SI)' },
    { id:'gars_rrb', type:'std_40_160', prompt:'Restricted / Repetitive (RRB)' },
    { id:'gars_cib', type:'std_40_160', prompt:'Communication/Interaction (CIB)' },
    { id:'gars_eis', type:'std_40_160', prompt:'Emotional/Irregularity (EIS)' },
    { id:'gars_ai',  type:'std_40_160', prompt:'Autism Index (AI)' },
  ],
};

// ---- 2.2 UI renderers for control types ----
function renderControl(item){
  ...
  switch(item.type){
    case 'likert_0_3':
      return `
        <select id="${id}">
          <option value="">--</option><option>0</option><option>1</option><option>2</option><option>3</option>
        </select>`;
    case 'cars_1_4':
      return `<input id="${id}" type="number" step="0.5" min="1" max="4">`;
    case 'num_0_3':
      return `<input id="${id}" type="number" min="0" max="3">`;
    case 'std_40_160':
      return `<input id="${id}" type="number" min="40" max="160">`;
    case 'scaled_1_19':
      return `<input id="${id}" type="number" min="1" max="19">`;
    default:
...
function renderSection(title, items){
  ...
  return `
    <div class="mini-section">
      ...
      <table class="table"><thead><tr><th>Item</th><th>Response</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>`;
}

// ---- 2.3 Start button → build one merged form ----
document.addEventListener('DOMContentLoaded', () => {
  const startBtn = U.el('#u_psych_start');
  if(!startBtn) return;

  startBtn.addEventListener('click', () => {
    const selected = {
      wisc:  U.el('#u_sel_wisc')?.checked,
      sb5:   U.el('#u_sel_sb5')?.checked,
      gars3: U.el('#u_sel_gars3')?.checked,
      ados2: U.el('#u_sel_ados2')?.checked,
      cars2: U.el('#u_sel_cars2')?.checked,
    };

    const host = U.el('#psych_unified_form');
    if(!host) return;

    // Build unified UI
    let html = '';
    html += renderSection('Shared Observations (asked once)', ITEM_BANKS.shared);

    if(selected.ados2){
      // Module selector + ADOS items
      html += `
        <div class="mini-section">
          <h3 class="sub-heading">ADOS-2 Module</h3>
          <select id="u_ados_module">
            <option value="">-- Select Module --</option>
            ${ITEM_BANKS.ados2.modules.map(m=>`<option>${m}</option>`).join('')}
          </select>
        </div>`;
      html += renderSection('ADOS-2 Items', ITEM_BANKS.ados2.items);
      html += `
        <div class="mini-section">
          <label>ADOS Administration Notes</label>
          <textarea id="u_ados_notes" rows="3"></textarea>
        </div>`;
    }

    if(selected.cars2) html += renderSection('CARS-2', ITEM_BANKS.cars2);
    if(selected.wisc)  html += renderSection('Wechsler (WISC-V / WAIS)', ITEM_BANKS.wisc);
    if(selected.sb5)   html += renderSection('SB-5', ITEM_BANKS.sb5);
    if(selected.gars3) html += renderSection('GARS-3', ITEM_BANKS.gars3);

    // Pull in your existing “Observations During Assessment” + IEP goals (no duplication)
    html += `
      <div class="mini-section">
        <h3 class="sub-heading">Session Observations</h3>
        <label>Breaks required?</label>
        <select id="u_breaks_req"><option value="no">No</option><option value="yes">Yes</option></select>
        <label>If yes, count</label>
        <input id="u_breaks_count" type="number" min="0">
        <label>Engagement (1–5)</label>
        <input id="u_engagement" type="range" min="1" max="5" value="4">
        <label>Clinician summary</label>
        <textarea id="u_obs_summary" rows="4"></textarea>
      </div>
      <div class="mini-section">
        <h3 class="sub-heading">IEP Goal Domains</h3>
        ${['communication','social','emotional','executive','dailyLiving','behavior','academics'].map(v=>`
          <label><input type="checkbox" name="u_iep[]" value="${v}"> ${v}</label>
        `).join('')}
        <label style="display:block;margin-top:6px;">Other</label>
        <textarea id="u_iep_other" rows="2"></textarea>
      </div>
      <div class="mini-section">
        <button id="u_psych_generate" class="btn btn-primary">Generate Psychological Narrative (AI)</button>
      </div>
    `;
    ...
    // Wire generate
    U.el('#u_psych_generate').addEventListener('click', async ()=>{
      try{
        toggleLoading(true);
        const payload = collectUnifiedPsych();
        await generatePsychReport(payload); // reuse your existing AI flow
      } catch(err){
        console.error(err);
...
// ---- 2.4 Collector + basic scoring stubs ----
function readResponses(items){
  const out = {};
  items.forEach(it=>{
    const v = U.val(U.el(`#u_${it.id}`));
    out[it.id] = v === '' ? null : (isFinite(+v) ? +v : v);
  });
  return out;
}

function collectUnifiedPsych(){
  const selected = {
    wisc:  U.el('#u_sel_wisc')?.checked || false,
    sb5:   U.el('#u_sel_sb5')?.checked || false,
    gars3: U.el('#u_sel_gars3')?.checked || false,
    ados2: U.el('#u_sel_ados2')?.checked || false,
    cars2: U.el('#u_sel_cars2')?.checked || false,
  };

  const data = {
    _meta:{
      generatedAt: new Date().toISOString(),
      selected: Object.entries(selected).filter(([,v])=>v).map(([k])=>k),
      module: U.val(U.el('#u_ados_module')) || null
    },
    shared: readResponses(ITEM_BANKS.shared),
    instruments:{},
    observations:{
      breaksRequired: U.val(U.el('#u_breaks_req')) || null,
      breaksCount:    U.val(U.el('#u_breaks_count')) || null,
      engagement1to5: U.val(U.el('#u_engagement')) || null,
      summary:        U.val(U.el('#u_obs_summary')) || ''
    },
    iepGoals:{
      selected: U.els('input[name="u_iep[]"]:checked').map(i=>i.value),
      other:    U.val(U.el('#u_iep_other')) || ''
    }
  };

  if(selected.ados2){
    data.instruments.ados2 = {
      module: data._meta.module,
      items: readResponses(ITEM_BANKS.ados2.items),
      notes: U.val(U.el('#u_ados_notes')) || ''
    };
    data.instruments.ados2.summary = scoreADOS2(data.instruments.ados2);
  }
  if(selected.cars2){
    data.instruments.cars2 = {
      items: readResponses(ITEM_BANKS.cars2),
      summary: scoreCARS2(readResponses(ITEM_BANKS.cars2))
    };
  }
  if(selected.wisc){
    data.instruments.wisc = {
      indices: readResponses(ITEM_BANKS.wisc),
      summary: scoreWISC(readResponses(ITEM_BANKS.wisc))
    };
  }
  if(selected.sb5){
    data.instruments.sb5 = {
      subtests: readResponses(ITEM_BANKS.sb5),
      summary: scoreSB5(readResponses(ITEM_BANKS.sb5))
    };
  }
  if(selected.gars3){
    data.instruments.gars3 = {
      scales: readResponses(ITEM_BANKS.gars3),
      summary: scoreGARS3(readResponses(ITEM_BANKS.gars3))
    };
  }

  // expose for debugging
  window.PSYCH_UNIFIED_DEBUG = data;
  return data;
}

// ---- 2.5 Scoring stubs (generic). Plug in official rules you’re licensed to use. ----
function safeNums(obj){ return Object.values(obj).map(v=>+v).filter(v=>Number.isFinite(v)); }
function scoreCARS2(items){
  const arr = safeNums(items);
  const total = arr.reduce((a,b)=>a+b,0);
  return { itemsEntered: arr.length, total: arr.length? +total.toFixed(2): null };
}
function scoreADOS2(ados){
  const arr = safeNums(ados.items);
  const total = arr.reduce((a,b)=>a+b,0);
  return { totalEntered: arr.length? +total.toFixed(2): null };
}
function scoreWISC(idx){
  const n = safeNums(idx);
  const fsiq = Number(idx.wisc_fsiq) || null;
  const mean = n.length ? +(n.reduce((a,b)=>a+b,0)/n.length).toFixed(1) : null;
  return { fsiq, meanIndex: mean };
}
function scoreSB5(sub){
  const n = safeNums(sub);
  return { subtestsEntered: n.length };
}
function scoreGARS3(scales){
  const ai = Number(scales.gars_ai) || null;
  return { autismIndex: ai };
}
/* band used for 0–3 CAAT-Psych domain means */
function bandFromMean(mean){
  if (mean == null) return null;  // 0–3 scale
  if (mean < 0.5)  return "Low concern";
  if (mean < 1.2)  return "Mild";
  if (mean < 2.0)  return "Moderate";
  return "High";
}

/*************** CAAT-PSYCH: Expanded Original Item Bank ***************/
const CAAT_PSYCH_BANK = {
  // Uniform concern scale (applies to every item)
  // 0 = Not observed/no concern • 1 = Mild • 2 = Moderate • 3 = Severe/Frequent
  scale: {
    min: 0,
    max: 3,
    labels: ["0 – Not observed", "1 – Mild", "2 – Moderate", "3 – Severe/Frequent"],
    notes: "Rate level of concern observed during assessment/caregiver report."
  },

  domains: {
    /* 1) SOCIAL COMMUNICATION (10) */
    social_comm: {
      title: "Social Communication",
      items: [
        { id:"sc_eye_limited",  prompt:"Limited spontaneous eye contact during interaction" },
        { id:"sc_joint_attention", prompt:"Reduced shared enjoyment/joint attention (showing/pointing to share)" },
        { id:"sc_resp_name",    prompt:"Inconsistent response to name or social bids" },
        { id:"sc_reciprocity",  prompt:"Weak social reciprocity (back-and-forth engagement)" },
        { id:"sc_facial_cues",  prompt:"Difficulty reading facial expressions or social cues" },
        { id:"sc_gestures",     prompt:"Limited use/understanding of gestures (pointing, nodding)" },
        { id:"sc_initiation",   prompt:"Rarely initiates social interaction with adults/peers" },
        { id:"sc_turn_taking",  prompt:"Difficulty with turn-taking in conversation or play" },
        { id:"sc_peer_rel",     prompt:"Limited peer relationships or interest in peers" },
        { id:"sc_prosody",      prompt:"Atypical prosody (monotone/sing-song/odd rhythm) impacting communication" }
      ]
    },

    /* 2) RESTRICTED/REPETITIVE BEHAVIORS (10) */
    rrb: {
      title: "Restricted / Repetitive Behaviors",
      items: [
        { id:"rrb_stereotypy",  prompt:"Repetitive movements (hand-flapping, rocking, finger movements)" },
        { id:"rrb_repetitive_use", prompt:"Repetitive use of objects (spinning, lining up, tapping)" },
        { id:"rrb_fixated_int", prompt:"Intense/fixated interests (topics, objects, media)" },
        { id:"rrb_rigidity",    prompt:"Insistence on sameness or rigid routines" },
        { id:"rrb_change_distress", prompt:"Distress with changes in schedule, materials, or environment" },
        { id:"rrb_visual_inspect", prompt:"Unusual visual inspection (peripheral gaze, close-looking)" },
        { id:"rrb_sens_seek",   prompt:"Sensory seeking (crashing, sniffing, mouthing, touching repeatedly)" },
        { id:"rrb_sens_avoid",  prompt:"Sensory avoidance (covers ears, avoids textures, light sensitivity)" },
        { id:"rrb_restricted_play", prompt:"Restricted play repertoire (same theme/toy use repeatedly)" },
        { id:"rrb_routines_rules", prompt:"Over-adherence to rules/rituals; upset if steps change" }
      ]
    },

    /* 3) LANGUAGE / COMMUNICATION (10) */
    language: {
      title: "Language / Communication",
      items: [
        { id:"lang_expressive_low", prompt:"Limited expressive vocabulary or phrase length for age" },
        { id:"lang_receptive",      prompt:"Difficulty understanding simple directions/questions" },
        { id:"lang_multistep",      prompt:"Difficulty following 2–3 step directions" },
        { id:"lang_echolalia",      prompt:"Echolalia/scripting that interferes with communication" },
        { id:"lang_pragmatics",     prompt:"Pragmatic difficulties (topic maintenance, relevance, repair)" },
        { id:"lang_requesting",     prompt:"Needs prompting to request/communicate needs" },
        { id:"lang_questions",      prompt:"Difficulty answering WH-questions (who/what/where/why)" },
        { id:"lang_narrative",      prompt:"Difficulty recounting events or telling simple stories" },
        { id:"lang_intelligibility",prompt:"Speech intelligibility limits listener understanding" },
        { id:"lang_aac_needs",      prompt:"Needs AAC/gestures/sign for functional communication" }
      ]
    },

    /* 4) PLAY / IMAGINATION (8) */
    play: {
      title: "Play / Imagination",
      items: [
        { id:"play_functional",  prompt:"Limited functional play with toys/materials" },
        { id:"play_symbolic",    prompt:"Limited pretend/symbolic play (feeding doll, role play)" },
        { id:"play_turn_games",  prompt:"Difficulty with simple turn-taking games" },
        { id:"play_shared",      prompt:"Limited shared play with peers or adults" },
        { id:"play_repetitive",  prompt:"Repetitive play themes or focus on parts of objects" },
        { id:"play_expand",      prompt:"Difficulty expanding play themes without adult support" },
        { id:"play_generalize",  prompt:"Difficulty generalizing play skills across settings/toys" },
        { id:"play_rule_flex",   prompt:"Inflexible about play rules; upset if rules vary" }
      ]
    },

    /* 5) ATTENTION / EXECUTIVE FUNCTION (10) */
    attention_ef: {
      title: "Attention / Executive Function",
      items: [
        { id:"ef_distractible",  prompt:"Easily distracted by sounds/visuals/people" },
        { id:"ef_sustain",       prompt:"Difficulty sustaining attention to non-preferred tasks" },
        { id:"ef_initiation",    prompt:"Needs prompting to start tasks/activities" },
        { id:"ef_shift",         prompt:"Difficulty shifting between tasks; perseverates on one" },
        { id:"ef_workmem",       prompt:"Weak working memory (forgets steps, needs re-teaching)" },
        { id:"ef_plan_org",      prompt:"Difficulty planning/organizing materials or steps" },
        { id:"ef_impulsivity",   prompt:"Impulsive responses; acts before thinking" },
        { id:"ef_processing",    prompt:"Slow processing speed compared with peers" },
        { id:"ef_transition",    prompt:"Difficulty with transitions; needs countdown/visuals" },
        { id:"ef_follow_rules",  prompt:"Needs frequent prompts to follow classroom/home rules" }
      ]
    },

    /* 6) EMOTIONAL REGULATION (8) */
    emotion_reg: {
      title: "Emotional Regulation",
      items: [
        { id:"er_frustration",  prompt:"Low frustration tolerance; escalates with minor demands" },
        { id:"er_recovery",     prompt:"Slow to recover after distress without adult co-regulation" },
        { id:"er_mood_var",     prompt:"Frequent mood variability during session/day" },
        { id:"er_waiting",      prompt:"Difficulty waiting or tolerating delays" },
        { id:"er_anxiety_signs",prompt:"Observable anxiety (avoidance, tension, reassurance seeking)" },
        { id:"er_triggers",     prompt:"Strong emotional reactions to specific triggers (sensory, change)" },
        { id:"er_coping_rigid", prompt:"Limited coping strategies; rigid responses to stress" },
        { id:"er_safety_risk",  prompt:"Escalation risks (aggression/self-injury/bolting) when dysregulated" }
      ]
    },

    /* 7) SENSORY PROCESSING (12) */
    sensory: {
      title: "Sensory Processing",
      items: [
        { id:"sens_aud_hyper",  prompt:"Auditory over-responsivity (covers ears/avoids noise)" },
        { id:"sens_aud_hypo",   prompt:"Auditory under-responsivity (reduced response to sounds)" },
        { id:"sens_vis_hyper",  prompt:"Visual over-responsivity (bright lights/motion distress)" },
        { id:"sens_vis_hypo",   prompt:"Visual under-responsivity (reduced response to visual cues)" },
        { id:"sens_tac_def",    prompt:"Tactile defensiveness (grooming/clothing/texture aversion)" },
        { id:"sens_tac_seek",   prompt:"Tactile seeking (touching, rubbing, messy play seeking)" },
        { id:"sens_vest_seek",  prompt:"Vestibular seeking (spinning, jumping, movement craving)" },
        { id:"sens_vest_avoid", prompt:"Vestibular avoidance (fear of swings, movement sensitivity)" },
        { id:"sens_prop_seek",  prompt:"Proprioceptive seeking (crashing, heavy work seeking)" },
        { id:"sens_oral",       prompt:"Oral seeking/avoidance (mouthing, chewing, gagging)" },
        { id:"sens_olfactory",  prompt:"Atypical smell behaviors or strong odor sensitivity" },
        { id:"sens_intero",     prompt:"Interoception difficulty (hunger/toileting/body signals)" }
      ]
    },

    /* 8) MOTOR (8) */
    motor: {
      title: "Motor (Fine/Gross)",
      items: [
        { id:"mot_core",     prompt:"Low core/postural stability" },
        { id:"mot_balance",  prompt:"Balance difficulties (falls, unsteady gait)" },
        { id:"mot_praxis",   prompt:"Motor planning difficulties (learning new actions/sequences)" },
        { id:"mot_bilateral",prompt:"Weak bilateral coordination (two-hand use)" },
        { id:"mot_fm_dex",   prompt:"Fine motor dexterity difficulty (small object manipulation)" },
        { id:"mot_graphomotor", prompt:"Graphomotor/handwriting control difficulty" },
        { id:"mot_speed",    prompt:"Slow/inefficient motor execution for age" },
        { id:"mot_dominance",prompt:"Hand dominance unclear/mixed beyond expected age" }
      ]
    },

    /* 9) ADAPTIVE / DAILY LIVING (10) */
    adaptive: {
      title: "Adaptive / Daily Living",
      items: [
        { id:"ad_toileting", prompt:"Toileting independence below expected level" },
        { id:"ad_dressing",  prompt:"Dressing (on/off, fasteners) requires more help than peers" },
        { id:"ad_hygiene",   prompt:"Hygiene routines (wash, brush teeth, grooming) need support" },
        { id:"ad_feeding",   prompt:"Feeding/eating skills (utensils, cup/straw) below expectations" },
        { id:"ad_sleep",     prompt:"Sleep regulation concerns (falling/staying asleep, early waking)" },
        { id:"ad_safety",    prompt:"Limited safety awareness (elopement, hazards, stranger safety)" },
        { id:"ad_routines",  prompt:"Difficulty following home/school routines without prompts" },
        { id:"ad_community", prompt:"Community participation limitations (tolerating outings, waiting)" },
        { id:"ad_chores",    prompt:"Limited participation in age-appropriate chores/tasks" },
        { id:"ad_time_mgmt", prompt:"Difficulty with time awareness/schedule management" }
      ]
    },

    /* 10) LEARNING / ACADEMICS (10) */
    learning: {
      title: "Learning / Academics",
      items: [
        { id:"lr_imitation",  prompt:"Difficulty imitating modeled actions/sounds" },
        { id:"lr_match_sort", prompt:"Difficulty matching/sorting basic items" },
        { id:"lr_concepts",   prompt:"Delayed pre-academic concepts (colors, shapes, sizes)" },
        { id:"lr_letters",    prompt:"Letter recognition/writing delays" },
        { id:"lr_numbers",    prompt:"Number concepts/counting delays" },
        { id:"lr_phono",      prompt:"Phonological awareness difficulties (rhyme, sound blending)" },
        { id:"lr_reading",    prompt:"Reading decoding/comprehension below expectations" },
        { id:"lr_writing",    prompt:"Written expression organization/spelling difficulty" },
        { id:"lr_math",       prompt:"Math problem-solving/calculation difficulty" },
        { id:"lr_class_beh",  prompt:"Classroom learning behaviors (attention, follow directions) limited" }
      ]
    }
  }
};
/* === CAAT-Psych: Form renderer (creates Generate button) === */
function renderCaatPsychForm(selectedKeys){
  const host = document.getElementById('caat_psych_form');
  if (!host) return;
  const scale = CAAT_PSYCH_BANK.scale;

  // Build sections for each chosen domain
  const sections = selectedKeys.map(key=>{
    const d = CAAT_PSYCH_BANK.domains[key];
    if(!d) return '';
    const rows = d.items.map(it=>`
      <tr>
        <td>${it.prompt}</td>
        <td>
          <select id="cp_${it.id}" aria-label="${it.prompt}">
            <option value=""></option>
            ${scale.labels.map((lab,i)=>`<option value="${i}">${lab}</option>`).join('')}
          </select>
        </td>
        <td><input id="cp_${it.id}_notes" placeholder="Notes (optional)"></td>
      </tr>
    `).join('');
    return `
      <div class="mini-section">
        ...
        <table class="table">
          <thead><tr><th>Item</th><th>0–3 (concern)</th><th>Notes</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </div>`;
  }).join('');

  // Observations + IEP goals + Generate
  const tail = `
    <div class="mini-section">
      <h3 class="sub-heading">Session Observations</h3>
      <label>Engagement (1–5)</label>
      <input id="cp_engagement" type="range" min="1" max="5" value="4">
      <label>Clinician summary</label>
      <textarea id="cp_summary" rows="3"></textarea>
    </div>
    <div class="mini-section">
      <h3 class="sub-heading">IEP Goal Domains</h3>
      ${['communication','social','emotional','executive','dailyLiving','behavior','academics']
        .map(v=>`<label><input type="checkbox" name="cp_goals" value="${v}"> ${v}</label>`).join(' ')}
      <label style="display:block;margin-top:6px;">Other</label>
      <textarea id="cp_goals_other" rows="2"></textarea>
    </div>
    <div class="mini-section">
      <button id="btnGenerateCaatPsych" class="btn btn-primary">Generate Psychological Narrative (AI)</button>
    </div>`;

  host.innerHTML = sections + tail;
  host.style.display = 'block';

  // === Generate button handler (this is the “one more hookup”) ===
  document.getElementById('btnGenerateCaatPsych')?.addEventListener('click', async ()=>{
    const payload = collectCaatPsychPayload(selectedKeys);  // uses new bands
    toggleLoading?.(true);
    try { await generatePsychReport(payload); }
    catch (e){ console.error(e); alert('Could not generate report.'); }
    finally { toggleLoading?.(false); }
  });
}

/* === CAAT-Psych collector with per-domain bands === */
function collectCaatPsychPayload(selectedKeys){
  const domains = {};
  ...
  selectedKeys.forEach(key=>{
    ...
    const obj = {};
    d.items.forEach(it=>{
      const v = document.getElementById(`cp_${it.id}`)?.value ?? '';
      const n = v === '' ? null : Number(v);
      obj[it.id] = Number.isFinite(n) ? n : null;
      obj[`${it.id}_notes`] = document.getElementById(`cp_${it.id}_notes`)?.value?.trim() || '';
    });

    const scores = Object.values(obj).filter(v=>typeof v==='number');
    const total  = scores.reduce((a,b)=>a+b,0);
    const mean   = scores.length ? +(total/scores.length).toFixed(2) : null;

    domains[key] = {
      title: d.title,
      responses: obj,
      summary: {
        itemsEntered: scores.length,
        mean,
        total,
        band: bandFromMean(mean)  // <- per-domain band
      }
    };
  });

  // Composite
  const domainMeans = Object.values(domains).map(d=>d.summary.mean).filter(v=>typeof v==='number');
  const composite   = domainMeans.length ? +(domainMeans.reduce((a,b)=>a+b,0)/domainMeans.length).toFixed(2) : null;

  return {
    _meta: {
      generatedAt: new Date().toISOString(),
      instrument: "CAAT-Psych (Original)",
      scale: "0=Not observed, 3=Severe/Frequent",
      selectedDomains: selectedKeys
    },
    domains,
    composite: { mean: composite, band: bandFromMean(composite) },
    observations: {
      engagement1to5: document.getElementById('cp_engagement')?.value ?? null,
      summary: document.getElementById('cp_summary')?.value?.trim() || ''
    },
    iepGoals: {
      selected: Array.from(document.querySelectorAll('input[name="cp_goals"]:checked')).map(i=>i.value),
      other: document.getElementById('cp_goals_other')?.value?.trim() || ''
    }
  };
}

/* === Launcher wiring (so “Start CAAT-Psych” works) === */
function getSelectedDomainsFromTable() {
  return Array.from(document.querySelectorAll('#caatPsychDomainTable input[type="checkbox"][data-domain]:checked'))
    .map(cb => cb.dataset.domain);
}
function handleStartCaatPsych(e){
  e?.preventDefault?.();
  const selected = getSelectedDomainsFromTable();
  if (!selected.length) { alert('Select at least one domain.'); return; }
  document.getElementById('legacy_instruments')?.classList.add('hidden'); // optional
  renderCaatPsychForm(selected);
  document.getElementById('caat_psych_form')?.scrollIntoView({ behavior:'smooth', block:'start' });
}
function initCaatPsychLauncher(){
  const startBtn = document.getElementById('btnStartCaatPsych');
  if (startBtn) startBtn.addEventListener('click', handleStartCaatPsych);

  const selAll = document.getElementById('caatPsychSelectAll');
  if (selAll) selAll.addEventListener('change', ()=>{
    document.querySelectorAll('#caatPsychDomainTable input[type="checkbox"][data-domain]')
      .forEach(cb => cb.checked = selAll.checked);
  });
  document.querySelectorAll('#caatPsychDomainTable input[type="checkbox"][data-domain]').forEach(cb=>{
    cb.addEventListener('change', ()=>{
      const all = document.getElementById('caatPsychSelectAll');
      if (!all) return;
      const boxes = Array.from(document.querySelectorAll('#caatPsychDomainTable input[type="checkbox"][data-domain]'));
      all.checked = boxes.every(x => x.checked);
    });
  });
}
document.addEventListener('DOMContentLoaded', initCaatPsychLauncher);
